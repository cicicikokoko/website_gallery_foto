    ...
    if (isset($_FILES['new_photo']) && $_FILES['new_photo']['error'] == UPLOAD_ERR_OK) {
        ...
        if($check !== false) {
            ...
            if (move_uploaded_file($_FILES["new_photo"]["tmp_name"], $target_file)) {
                ...
                $update_file_query = "UPDATE foto SET LokasiFile = ? WHERE FotoID = ?";
                $stmt = $conn->prepare($update_file_query);
                $stmt->bind_param("si", $target_file, $foto_id);
...
<body class="bg-gray-100">
    <div class="container mx-auto p-6">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                <label for="current_photo" class="block text-gray-700">Foto Saat Ini:</label>
                <img src="<?php echo $foto['LokasiFile']; ?>" alt="Foto saat ini" class="max-w-xs mt-2">
            </div>
            <div class="mb-4">
                <label for="new_photo" class="block text-gray-700">Ganti Foto (opsional):</label>
                <input type="file" name="new_photo" id="new_photo" class="border rounded p-2 w-full">
            </div>
            <button type="submit" class="bg-blue-500 text-white py-2 px-4 rounded">Simpan Perubahan</button>
            <a href="dashboard.php" class="bg-gray-500 text-white py-2 px-4 rounded ml-2">Batal</a>
        </form>
    </div>
</body>
</html>
